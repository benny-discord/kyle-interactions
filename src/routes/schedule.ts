import type { BennyStatusResponse, reqEnv, Shard } from "../types";

export default async function(_: any, env: reqEnv) {
	...
	if (data) {
		if (data.status == 200) {
			...
			if (!shards.find(x => x.status != 0)) {
				...
				const date = new Date();
				await env.GRAPH_STORAGE.put(`${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getDate().toString().padStart(2, '0')}`, '', {
