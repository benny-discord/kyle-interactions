export function parseToNumber(str: string): number | null {
	...
	return matches.map(m => parse(m.trim())).reduce((p, v) => p + v, 0);
};

export function formatToString(ms: number) {
	ms = Math.abs(ms);
	if (!Number.isInteger(ms) || ms < 1) throw new Error('Valid Integer must be provided');
